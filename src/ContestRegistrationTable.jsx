...
const Navigate =useNavigate();
  ...
  const fetchSchools = async () => {
    setIsLoading(true)
    try {
      const response = await fetch("https://heritage-server-389286764509.asia-south1.run.app/events")
      if (!response.ok) {
...
setFilteredSchools(data)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://heritage-server-389286764509.asia-south1.run.app/contest/register", {
        method: "POST",
        headers: {
